...
Local CLIs only; synchronous (no background jobs). (Package id `dex`; internal config/dir still use `gavel` — `~/.gavel`, `GAVEL_*`, `scripts/gavel.mjs`, dir `gavel-main` — a deferred full internal rename.)
...
- `commands/` — slash commands (`auto`, `debate`, `ask`, `eval`, `setup`, `config`); thin Claude-side wrappers.
  ...
  - **`fuse` (standalone 1-round naive synthesis) and the naive-majority ENSEMBLE are DISABLED** (`commands/fuse.md.disabled`): measured WORST in both regimes (78% easy, **38% hard** — weak voices outvote the strong one, dropping below a single good model and below Claude-alone). The panel is still queried in parallel by `auto`/`debate` via the `gavel.mjs fuse` **runner**, but answers are **judge-synthesized (quality-weighted) + verified**, never majority-voted.
...
- `scripts/eval.mjs` — zero-dep **eval harness**: reads `~/.gavel/eval-set.json`, runs each item through
...
just another `gavel.mjs fuse --json` call with a constructed prompt). Round 0: Claude's blind draft.
...
models converge sycophantically). Claude synthesizes the final round per `gavel-synthesis` and reports
...
abstain on unknown). Measurement lives in `~/.gavel/eval-set.json` (run it to MEASURE, not estimate).
