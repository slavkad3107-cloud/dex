---
description: Run the machine-scored eval harness (ask/fuse) over ~/.dex/eval-set.json and print a stratified scorecard.
...
- For `ask <provider>`: `--mode ask --provider <provider>` (provider must be a real dex provider slug).
