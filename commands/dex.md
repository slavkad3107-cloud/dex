---
description: Alias for /dex:auto — cost-aware cascade (probe → panel → debate). Default entry point when no subcommand is given.
argument-hint: "<task or question>"
---

This is the default dex entry point. It behaves identically to `/dex:auto`.

If $ARGUMENTS is empty, ask what the user wants to do and stop.

Otherwise, follow the full `/dex:auto` logic below.

---

You are running **dex** (= **dex:auto**) — a cost-aware cascade. Spend model effort only when the
answer is uncertain. You (Claude) are panelist + judge + actor. Escalate through the stages below
and **STOP at the first stage that yields a confident answer**, then act. Get prompts to models via
a temp file + `--prompt-file --json` (never inline in the shell).
...
appropriate action. The point of `dex` is to reach Stage 3 only when the question actually warrants it.
